...
const Probability: NextPage = () => {
  ...
  targetData.forEach((item) =>
    initialProbability.push(calcProbability({ ...item, sq: startingBudget }))
  );
  const [targetSQ, setTargetSQ] = useState<number[]>(
    Array(targetData.length).fill(startingBudget)
  );
  const [probabilities, setProbabilities] =
    useState<number[]>(initialProbability);
  ...
  const handleProbabilityChange =
    (index: number, targetData: TargetDataType, npUpdate: number = 0) =>
    (_event: Event, newValue: number | number[]) => {
      ...
      newProbabilities[index] = calcProbability({
        ...targetData,
        sq: SQUsed,
        np: npSetting
      });
  ...
  const handleNpChange =
    (index: number, targetData: TargetDataType) =>
    ...
    <div className="probability-container">
      ...
      <div className="probability-calculations" ref={ref}>
        {targetData.map((item, index) => {
          ...
          if (item.type === TargetOptions.servant) {
            ...
            const targetBanner = servantData[
              servantName as keyof typeof servantData
            ]
              ? servantData[servantName as keyof typeof servantData].find(
                  (banner) => {
            ...
            maxSQForBanner = targetBannerDate
              ? calcTotalSQForDate({
                  excludeOptions,
                  currentSQ,
                  currentTickets,
          ...
          return (
            <div key={item.name} className="target-calc">
              <div className="target-settings">
                ...
                {item.type === TargetOptions.servant && (
                  <Select
                    ...
                    id="np-setting"
                    value={
                      npSettings[index] ? npSettings[index].toString() : "1"
                    }
                    label="Np Setting"
              ...
              {item.type === TargetOptions.servant && (
                <div>{`${item.rarity}* ${typeCopy} - Max ${maxSQForBanner} SQ available by ${targetBannerDate}`}</div>
              )}
              {item.type === TargetOptions.ce && (
              ...
              (item.type === TargetOptions.servant &&
                ...
                <div className="target-prob-container">
                  ...
                  <div className="target-prob-box">
                    ...
                    <div className="target-prob-percent">{`${Math.floor(
                      probabilities[index]
                    )}%`}</div>
                  </div>
                </div>
              ...
              {item.type === TargetOptions.servant && (
                <div className="banners">
                  ...
                  {servantData[servantName as keyof typeof servantData] &&
                    servantData[servantName as keyof typeof servantData].map(
                      (banner) => {
                        const bannerDateAsMoment = moment(
                          banner[1],
                          "YYYY/MM/DD"
                        );
          ...
          <div className="spendings">
            ...
            <div className="sq-spent">
              ...
              <span className="total-spent">
                {targetSQ.length &&
                  targetSQ.reduce((acc, current) => acc + current, 0)}
              </span>
            </div>
            <div
              className={classNames({
                "yea-a-whale":
                  targetSQ.length && totalSQForBanner - totalSpent < 0,
                "not-a-whale":
                  targetSQ.length && totalSQForBanner - totalSpent > 0
              })}
            >{`(${targetSQ.length && totalSQForBanner - totalSpent})`}</div>
          </div>
        </div>
      </div>
      <Footer stepNum={3} linkTo="/" linkBack="/rolltarget" />
      <div className="probabilityDisclaimer">
        {copy["rateup"]["chance"]["subheaderDisclaimer"]}
      </div>
      <div className="credits-container">
        ...
        <div className="credits">
          ...
          <a href="https://docs.google.com/spreadsheets/d/1rKtRX3WK9ZpbEHhDTy7yGSxYWIav1Hr_KhNM0jWN2wc/edit#gid=0">
            {copy["credits"]["bannersLink"]}
          </a>
          <span>{copy["credits"]["event"]}</span>
          <a href="https://docs.google.com/spreadsheets/d/1qvxLU407QwiFaCvItqR16SqqAVlLD5u5nBzY_bCFYvs/edit#gid=2111060397">
            {copy["credits"]["eventLink"]}
          </a>
          <span>{copy["credits"]["wiki"]}</span>
          <a href="https://fategrandorder.fandom.com/wiki/Fate/Grand_Order_Wikia">
            {copy["credits"]["wikiLink"]}
          </a>
        </div>
      </div>
      <div className="disclaimer">
        <span>{copy["disclaimer"]}</span>
        <a href="mailto:fategrandsavings@gmail.com">{copy["contact"]}</a>
      </div>
    </div>
  );
};
